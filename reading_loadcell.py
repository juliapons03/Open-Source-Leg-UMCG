"""
Load Cell Demo and Logging Script

This script demonstrates reading and logging forces and moments from different
types of Dephy load cells using OpenSourceLeg utilities.

Features:

1. Load Cell Types Supported:
   - I2C-based DephyLoadcellAmplifier
   - ActPack-integrated load cells
   - NBLoadcellDAQ via SPI

2. Calibration: Automatically calibrates each load cell at startup to
   remove offsets.

3. Data Logging: Uses OpenSourceLeg's Logger to record Fx, Fy, Fz, Mx, My,
   and Mz in real time.

4. Real-Time Loop: Runs at 200 Hz using SoftRealtimeLoop for consistent
   sampling.

5. Flexible Demo Functions: Choose between `demo_loadcell_i2c`,
   `demo_loadcell_actpack`, or `demo_loadcell_nb_daq` depending on hardware.

Usage:
- Initialize Logger and SoftRealtimeLoop.
- Call the appropriate demo function for your load cell hardware.
"""
import numpy as np
